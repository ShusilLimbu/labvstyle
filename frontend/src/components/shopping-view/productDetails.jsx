...
const ProductDetailsDialog = ({ open, setOpen, productDetails }) => {
  ...
  function handleAddReview(){
   ...
   console.log('Sending review data:', {
    ...
    dispatch(addReview({
      ...
      if(data.type === '/order/addReview/fulfilled'){
        setRating(0);
        setReviewMsg('');
  ...
  useEffect(()=>{
    ...
    if (productDetails !== null) {
        console.log('Fetching reviews for product:', productDetails?._id); // ✅ Debug log
        dispatch(getReviews(productDetails?._id)).then((data) => {
            console.log('Reviews fetched:', data); // ✅ See reviews response
            console.log('Reviews payload:', data.payload); // ✅ See reviews data
        });
    }
  ...
  // Add this to see the reviews state
useEffect(() => {
    console.log('Reviews from state:', reviews); // ✅ See what's in Redux state
}, [reviews]);

  const averageReview = reviews && reviews.length> 0 ?
  reviews.reduce((sum, reviewItem)=> sum + (reviewItem.reviewValue || 0),0) / reviews.length : 0;
