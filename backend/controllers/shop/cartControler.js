    ...
    try {
        ...
        if(!userId || !productId || quantity <= 0){
            return res.status(400).json({
                success: false,
                message: 'Datos Provistos Invalidos'
            })
        }
...
const fetchCartItems = async(req,res)=>{
    try {
        ...
        const cart = await Cart.findOne({userId}).populate({
            ...
            select: "image title price salPrice"
        });
        ...
        res.status(500).json({
            success: false,
            message: 'No se pudo agregar el producto requerido'
        });
...
const updateCartItem = async(req,res)=>{
    try {
        ...
        const populateCartItems = cart.items.map((item)=>({
            ...
            title: item.productId ? item.productId.title: 'Producto no encontrado',
